...
# Ingestion-completeness CLI (Goal 12) — "what data do we have, what period does it
# cover, what's missing"; --deep also scans for year-gaps on >1M-row tables (slower)
...
.venv/bin/python scripts/nessus.py data validate     [--deep] [--json]   # exits 1 if any check fails
...
`api/scheduler.py` — APScheduler `AsyncIOScheduler`, started in FastAPI lifespan. ~36 jobs across the two source tiers, cadence retuned in **Goal 11** ("switch each connector into incremental mode") to match Tasks.md's source-update-frequency policy:

| Job ID(s) | Cadence | Source |
|---|---|---|
| `bills_daily`, `gazette_weekly`, `hansard_search`, `ocl_monthly`, `ocl_registrations`, `tribunal_decisions` | Daily | LEGISinfo, Canada Gazette RSS, Hansard sector mentions, OCL comms + registrations, CRTC decisions |
| `appointments_weekly`, `contracts_monthly`, `grants_quarterly`, `donations_quarterly`, `parliament_seed` | Weekly | GIC Appointments, Proactive Disclosure, Grants & Contributions, Elections Canada donations, MP roster |
| `gc_news`, `gazette_notices`, news connectors (e.g. `the_conversation_ca`) | Every 4 hours | GC news API, per-instrument Gazette notices, licensed Canadian news RSS |
| `iaac`, `cer_applications`, `orders_in_council` | Daily | IAAC active projects, CER active proceedings, Orders in Council |
| `statcan`, `bank_of_canada` | Every business day | StatCan cube catalogue, BoC Valet series catalogue |
| `cer`, `transport`, `geospatial`, `ckan_catalogue` | Weekly | CER incidents, Transport/geospatial catalogues, full Open Government CKAN crawl |
| `npri` | Monthly | NPRI facility releases (release-driven, was over-polled weekly) |
| government-dept RSS feeds (`pmo_news`, `boc_news`, `nrcan_news`, ... 9 total) | Daily | `pipeline/feeds.py` generic connector |
| `canadabuys_tenders` | Every 4 hours | CanadaBuys rolling tender-notice snapshots (raw-archive only, Goal 11) |

Core jobs are hand-written `_run_*` functions; breadth jobs are generated by `_register_breadth_connectors()` from `pipeline/connectors.py`. Every run writes a row to `scheduler_log` (`status`: `running`/`ok`/`error`/**`skipped`** — see conditional fetch below). Trigger manually: `POST /api/scheduler/trigger/{job_id}` (runs in a FastAPI `BackgroundTask`). Status: `GET /api/scheduler/status`. Jobs that ingest **embedded** sources call `_rebuild_search_index()` on success.

**Conditional fetch (Goal 11) — `pipeline/conditional_fetch.py`:** the full-snapshot bulk jobs (`contracts_monthly`, `donations_quarterly`, `grants_quarterly`, `ocl_monthly`, `ocl_registrations`, `appointments_weekly`) now fingerprint the upstream resource (CKAN `package_show` metadata, or an HTTP HEAD for non-CKAN URLs like the Elections Canada ZIP) *before* downloading, and skip the run entirely (`status="skipped"`, zero rows) if it matches the last recorded fingerprint. This is what makes the tighter cadences above safe — most scheduled fires cost one cheap metadata check, not a full re-pull. It fixed two real bugs in the process: `donations_quarterly`'s cache was reused forever and never actually refreshed past the first download, while `ocl_monthly` did the opposite — an unconditional force-redownload every single run. A fingerprint with every field unreadable (`None`) always fails open to "fetch it," never to "skip it."

**`pipeline/api_paginator.py`'s checkpoint "complete" boundary (Goal 11):** a checkpointed walk that stops via `stop_on_empty` (e.g. `connector_ckan_catalogue.py`'s full catalogue crawl, or `connector_orders_in_council.py`'s per-year walk) used to skip its terminal empty cursor forever on every later call — the exact position where new content appears once the source grows. A `status == "complete"` checkpoint now re-probes that one boundary cursor each call (cheap: one extra request) instead of locking it out permanently.

Two Goal-7 connectors (`canadabuys_tenders`, `bank_of_canada`) had working resumable raw-archival walks that were never wired into the scheduler at all until Goal 11 — both are still raw-archive-only (no `source_records`/typed-table rows yet). `bank_of_canada`'s walk has a known unresolved limitation: once every one of Valet's ~15,642 series has been walked once, the checkpoint skips all of them forever even though most gain new observations on a regular schedule (no per-series "since" parameter wired yet).

### Ingestion-completeness audit (Goal 12)

**`pipeline/data_audit.py`** answers "what data do we have, what period does it cover, and what's missing" from one computed snapshot — `build_inventory(session, deep=False)` — instead of cross-referencing `config/data-sources.yaml`, the scheduler dashboard, and a DB console by hand. It cross-references the yaml registry (category/priority/jurisdiction metadata) against **live** truth: DB row counts + earliest/latest dates per source (Tier 1 typed tables + breadth `source_records`, grouped by `source`), `scheduler_log` history (last run/success/error, failed-download counts), raw_storage manifests (files/bytes downloaded, duplicate fetches avoided, missing-on-disk integrity checks) and checkpoints (paginator gaps = "missing pages"; conditional-fetch fingerprints), and `shutil.disk_usage`. The yaml's own `enabled`/`last_checked` fields are treated as a documentation snapshot, never as live truth — `run_validation()` specifically flags drift between what the yaml claims and what `api.scheduler.SOURCE_CONFIGS` actually runs (it already caught one: yaml's `the_conversation_ca` vs the scheduler's `conversation_ca_politics` is the same source under two different ids).

Per-source backfill state (`full`/`partial`/`not_started`) is derived, never read from yaml prose: 0 rows → not started; a paginated checkpoint's `status` (`complete` + no open gaps → full, anything else → partial); otherwise partial only if the most recent ok/error scheduler run was an error that still committed rows (the `StreamLoadError` "dropped stream ≠ data lost" case above) — else full. "Missing years" (gaps inside a source's own observed date range) come from a generic `substr(date_col,1,4)` distinct-year scan, skipped by default on the two >1M-row tables (`contracts_monthly`, `donations_quarterly`) and on breadth sources over 5,000 rows per CLAUDE.md's full-scan performance rule — pass `deep=True` (CLI: `--deep`) to force them.

Three consumers share this one engine: **`scripts/nessus.py data {inventory,status,missing,backfill-status,disk-usage,validate}`** (human-readable tables or `--json`; `validate` exits 1 if any check fails — usable as a cron/CI gate), **`GET /api/data/health`** (`api/routes/data_health.py`, `?deep=true` supported), and the ops console's **"Data Completeness"** card (`frontend/index.html`).
...
- **Theming is CSS-based (Tailwind v4):** all tokens live in `app/globals.css` `@theme` — there is **no `tailwind.config`**. ⚠️ Changing an `@theme` token *value* requires `rm -rf web/.next` + restart; HMR serves stale compiled CSS otherwise (this has bitten twice). **Full design-system reference (colour, type scale, spacing, component/state standards): `web/design.md` — read it before adding a component or page.**
- **App shell** (`app/layout.tsx`): `AppTopBar` / (`AppSidebar` + scrolling `main`), wrapping every page (`h-screen overflow-hidden`, content scrolls in the middle). Components in `components/app-*.tsx`. (`AppTicker` exists in `components/app-ticker.tsx` but is not currently mounted in the layout.)
- **Two visual surfaces:** (1) this **"Intelligence Grade" terminal workspace** — off-white/parchment-tinted surfaces, Parliament Navy (`primary`) as the single dominant brand colour, muted green/amber/red semantic accents, Source Serif 4 headlines + Public Sans interface + JetBrains Mono data (`font-data-tabular` / `.mono`); (2) the **report "deliverable"** at `/briefings/[id]` only — light parchment + serif, using the `navy`/`parchment` brand tokens + `.briefing-prose`. Don't reuse the workspace `Scorecard` on the light reader (it has its own `LightScorecard`).
...
- **A new full-snapshot bulk source needs the conditional-fetch gate, not just a tighter cron.** Bumping a job's cadence (e.g. monthly → daily) without first wiring `pipeline/conditional_fetch.py`'s fingerprint check turns every extra fire into a full multi-hundred-MB re-download. See the Scheduler section above for the two real bugs this caught (`donations_quarterly` never refreshing; `ocl_monthly` force-redownloading every run).
- **A `walk_cursor_pages` source whose cursor space grows over time (offset pagination, per-year pages) needs `stop_on_empty=True` to ever reach a re-checkable "complete" state** — `stop_on_empty=False` (fixed finite cursor lists, e.g. CanadaBuys's 9 files, BoC's 15k series) never marks "complete" at all, so a cursor whose underlying content mutates in place is locked out after its first successful fetch with no boundary to re-probe. `connector_canadabuys.py:sync_rolling_tender_notices()` is the workaround pattern (bypass the checkpoint, refetch a known small subset directly) where that matters.
